...
function friendlyAuthError(err) {
  ...
  function loginOverlay(kind) {
    ...
    /* SHOW-PASSWORD TOGGLE.
       A hospital password gets typed on a ward keyboard with the lights half
       down, and an unreadable field is how a correct password gets reported as
       a broken sign-in. The eye is a plain button beside the input rather than
       a browser default, because Chrome only offers its own on some platforms
       and not at all once the field is scripted. */
    var EYE_OPEN =
    ...
    function wirePasswordEye(root) {
      (root || document).querySelectorAll("[data-ageye]").forEach(function (b) {
        if (b.getAttribute("data-wired")) return;
        b.setAttribute("data-wired", "1");
        b.addEventListener("click", function () {
          var input = document.getElementById(b.getAttribute("data-ageye"));
          if (!input) return;
    ...
    function draw(tab) {
      body.innerHTML =
        ...
        '<div class="ag-pw">' +
          '<input id="agPass" type="password" autocomplete="' +
            (tab === "up" ? "new-password" : "current-password") + '">' +
          '<button type="button" class="ag-eye" data-ageye="agPass" ' +
            'aria-label="Show password" aria-pressed="false" tabindex="0">' + EYE_OPEN + "</button>" +
        "</div>" +
        (tab === "up"
