...
window.AQIncidentDoc = (function () {
  ...
  function buildBody(inc, blank) {
    ...
    x += table(
      ...
      "</w:tr>" +
      // Patient identity: ALWAYS blank, never populated from the stored record, in both
      // the filled and blank versions of this form. These are completed in pen on the
      // printed copy and filed in the hospital's own records, which is what keeps
      // identifiable patient data out of the platform's database entirely. Passing `inc`
      // into these cells would quietly undo that, so they take a literal empty string.
      "<w:tr>" +
        labelCell("Name of patient", "", 4680) +
        labelCell("Patient ID / UHID number", "", 4680) +
      "</w:tr>" +
      "<w:tr>" +
        labelCell("Age", "", 3120) +
        labelCell("Gender", "", 3120) +
        labelCell("Ward / bed number", "", 3120) +
      "</w:tr>", { width: 9360 });
    ...
    x += spacer(140);
    x += para(run(
      "The patient identity fields in section 1 are intentionally left blank on every copy " +
      "of this form. Complete them in pen and file this sheet with your own records — " +
      "they are never stored in AQcredix. Generated by AQcredix. Adapt the wording to your organisation’s incident policy " +
