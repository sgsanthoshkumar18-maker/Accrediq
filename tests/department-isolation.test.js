...
check('no table is left out of both loops', () => {
  ...
  /* The hand-written exceptions. Each of these has a policy written out in
     full rather than generated by a loop, because each needs a rule the loops
     cannot express:
       - incidents    three-way visibility (reporter, responsible dept, quality)
       - attachments  inherits scope from the row it hangs off
       - audits       scoped on department_name, not department
       - bundle_audits governance override: is_admin() sees every ward, because
                      a compliance figure for one ward is not a compliance
                      figure. Ward staff still see only their own. */
  const accounted = new Set([].concat(DEPT_SCOPED, ORG_WIDE,
    Object.keys(CHILD_OF), ['incidents', 'attachments', 'audits', 'bundle_audits']));
